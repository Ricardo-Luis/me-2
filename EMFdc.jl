...
#> title = "Força Eletromotriz e Binário Eletromagnético da Máquina de Corrente Contínua"
#> description = "Apresenta-se a dedução da expressão da força eletromotriz (FEM) e do binário eletromagnético da máquina de corrente contínua em função dos seus parâmetros construtivos. É também analisada a ação do coletor e escovas no processo de retificação das FEM e comutação da corrente nas secções em movimento do enrolamento induzido (rotor)."
#> date = "2026-09-11"
...
TwoColumnWideLeft(md"`EMFdc.jl`", md"`Last update: 11·09·2026`")
...
md"""
A análise do coletor de escovas na ligação aos condutores do enrolamento induzido permite verificar que a sua ação realiza uma retificação de onda completa na FEM induzida em cada secção em movimento, Fig. 1.2. 

Compare as Figuras 1.1 e 1.2 e verifique a diferença na FEM induzida numa secção em movimento, quando ligada a anéis e escovas (Fig. 1.1), ou ligada a semi-anéis (coletor) e escovas (Fig. 1.2):
...
Fig. 1.1 -  FEM induzida numa secção em movimento com anéis e escovas. (Fonte: extraído de [^1])
"""
...
Fig. 1.2 -  FEM induzida numa secção em movimento com coletor e escovas. (Fonte: extraído de [^1])
"""
...
Fig. 2.1 -  Princípio de desenvovimento do binário eletromagnético num motor de corrente contínua. (a) Secção em movimento pela acção do binário; (b) Inversão da corrente (processo de comutação) ao passar pela linha neutra. (Fonte: extraído de [^2])
"""
...
[^1]: Moebs, W., Ling, S. & Sanny, J. "Electric Generators and Back Emf". In: University Physics - Volume 2 (OpenStax). Physics LibreTexts, 2026. [online] Disponível em: [https://phys.libretexts.org/@go/page/4433](https://phys.libretexts.org/@go/page/4433). Licença CC BY 4.0.


[^2]: Urone, P. P. & Hinrichs, R. "Force and Torque on a Current Loop". In: College Physics (OpenStax). Physics LibreTexts, 2026. [online] Disponível em: [https://phys.libretexts.org/@go/page/4433](https://phys.libretexts.org/@go/page/151568). Licença CC BY 4.0.
"""
...
md"""
!!! info "Informação"
	No índice deste *notebook*, os tópicos assinalado com "💻" requerem a participação do estudante.
"""
