...
#> section = 1
#> order = 1
#> image = "https://github.com/Ricardo-Luis/me-2/blob/af3d7de8eed339440ee9bfb86d703af4a835ea90/images/card/EqualArea.png?raw=true"
...
TwoColumnWideLeft(md"`EqualArea.jl`", md"`Last update: 01·12·2025`")
...
md"""
**Analise separadamente os dois alternadores síncronos trifásicos seguintes, cada um ligado em estrela a um barramento infinito de $6\; \rm{kV \;-}\;50\; \rm{Hz}$. Cada alternador fornece inicialmente $1\; \rm{MW}$ com $$\cos \varphi= 1$$.**


|  | Alternador 1 | Alternador 2|
|--:|:--:|:--:|
| Máquina síncrona: | polos lisos | polos salientes|
| Potência nominal : | $$5\; \rm{MVA}$$ | $$5\; \rm{MVA}$$ |
| Reatâncias/fase: | $\quad\qquad X_s=12.75\Omega \quad\qquad$ | $X_d=12.75\Omega$;  $X_q=\dfrac{X_d}{2}$ |
| Perdas: | desprezáveis | desprezáveis |
| FEM/fase: | $E_0=3.675 \rm{kV}$ | $E_0=3.675 \rm{kV}$ | 

\
**Suponha que ocorre uma falha no regulador da turbina, fazendo a potência mecânica, $P_{mec}$, aumentar subitamente de $1 \; \rm{MW}$ para um novo valor constante, $P_{mec} > 1\; \rm{MW}$.
...
md"""
O **critério da igualdade das áreas** é aplicável à máquina síncrona de polos salientes para verificar o limite de estabilidade dinâmica no seu funcionamento. No entanto, dado que a solução analítica é mais complexa, a integração numérica das áreas, obtida computacionalmente, permite verificar qual a carga máxima admissível da máquina síncrona, em regime transitório.
"""
...
manifest_format = "2.0"
project_hash = "01aad78f852858139bf484a19b9be4377e5240aa"
